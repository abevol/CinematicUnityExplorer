...

{
...

    {
        ...
        public override string Name => Localizer.Get("PANEL_LOG", "Log");
...
public static object GetLogSnapshot(int limit)
        {
            lock (Logs)
            {
                int count = Math.Min(limit, Logs.Count);
                var entries = new object[count];
                for (int i = 0; i < count; i++)
                {
                    var log = Logs[Logs.Count - count + i];
                    entries[i] = new { message = log.message, type = log.type.ToString() };
                }
                return entries;
            }
        }
...

        {
            ...
            ButtonRef clearButton = UIFactory.CreateButton(optionsRow, "ClearButton", Localizer.Get("BTN_CLEAR", "Clear"), new Color(0.2f, 0.2f, 0.2f));
            ...
            ButtonRef fileButton = UIFactory.CreateButton(optionsRow, "FileButton", Localizer.Get("BTN_OPEN_LOG_FILE", "Open Log File"), new Color(0.2f, 0.2f, 0.2f));
